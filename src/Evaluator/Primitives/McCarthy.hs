...
apply :: (MonadError EvalError m, MonadEnv m) => (Term -> m Term) -> DotList Term -> m Term
...
substituteValue arg term (List body) = List $ fmap f body
  where
    f (Symbol str) = if str == arg then term else Symbol str
    f (List xs) = substituteValue arg term (List xs)
    f (DotList xs) = substituteValue arg term (List xs)
    f t = t
